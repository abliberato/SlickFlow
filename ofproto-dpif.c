                        ...
                        bool check_stp)
{
    ...
    ovs_be16 pcp, flow_vlan_tci = ctx->flow.vlan_tci;
    ...
    pcp = ctx->flow.vlan_tci &  htons(VLAN_PCP_MASK); //Retira valor de VLAN_PCP de dentro de VLAN_TCI
    pcp = pcp>>5;
    pcp &= 7; 
      ...
      ////////////////////ipv4 tratamento de pacotes - inicio
  ...
  if (ctx->flow.dl_type == htons(ETH_TYPE_IP)) {
    ...
    if (ctx->flow.vlan_tci!=0){
      ...
      if (pcp==1){ //CAMINHO ALTERNATIVO

          
        if (ofport->up.pp.config==1){//descartar
            return;
        }
          
        else{
            
            ctx->flow.vlan_tci &= ~htons(VLAN_VID_MASK);//Limpa valor de VLAN_VID em VLAN_TCI
	    ctx->flow.vlan_tci &= ~htons(VLAN_PCP_MASK);//Limpa valor de VLAN_PCP em VLAN_TCI
            ctx->flow.vlan_tci |= htons(ctx->flow.dl_src[0]) | htons(VLAN_CFI)|32; // Insere em VLAN_TCI o numero da porta de envio do frame e sinaliza com VLAN_PCP = 1
	    
           
           ctx->flow.dl_src[0]=ctx->flow.dl_src[1];
           ctx->flow.dl_src[1]=ctx->flow.dl_src[2];
           ctx->flow.dl_src[2]=ctx->flow.dl_src[3];    
           ctx->flow.dl_src[3]=ctx->flow.dl_src[4];
           ctx->flow.dl_src[4]=ctx->flow.dl_src[5];
           ctx->flow.dl_src[5]=ctx->flow.dl_dst[0];
           ctx->flow.dl_dst[0]=ctx->flow.dl_dst[1];
      ...
      else{ //caminho principal
        
        if (ofport->up.pp.config==1){//trocar caminho
	   
	  len=ctx->flow.dl_src[0];			
	  ofp_port=ctx->flow.dl_src[1];
	  ofport = get_ofp_port(ctx->ofproto, ofp_port);
	  odp_port = ofp_port_to_odp_port(ofp_port);
            
          ctx->flow.vlan_tci &= ~htons(VLAN_VID_MASK);//Limpa valor de VLAN_VID em VLAN_TCI
	  ctx->flow.vlan_tci |= htons(ctx->flow.dl_src[2]) | htons(VLAN_CFI);// Insere em VLAN_TCI o numero da porta de envio do frame
            
          i1=3;
	  i2=0;
	  while (i1<=5) {
		ctx->flow.dl_src[i2++]=ctx->flow.dl_src[i1++];
	  }
	  i1=0;
	  while ((i1<=5)&&(i2<=5)) {
	 	ctx->flow.dl_src[i2++]=ctx->flow.dl_dst[i1++];
	  }
	  i2=0;
	  while (i1<=5) {
	 	ctx->flow.dl_dst[i2++]=ctx->flow.dl_dst[i1++];
	  }
          
	   ctx->flow.vlan_tci &= ~htons(VLAN_PCP_MASK);//Limpa valor de VLAN_PCP em VLAN_TCI
           ctx->flow.vlan_tci |= 32;//sinaliza com VLAN_PCP = 1
	   
   
          }
          
        else{ //porta de saida disponivel
            

	    int ult_salto=0;
	    len=ctx->flow.dl_src[0];
	    ult_salto=ctx->flow.vlan_tci&15;//Recebe o valor da porta inserido no VLAN_VID da regra de fluxo 
            ctx->flow.vlan_tci &= ~htons(VLAN_VID_MASK);//Limpa valor de VLAN_VID 
	    if (len>=5){
            	ctx->flow.vlan_tci |= htons(ctx->flow.dl_dst[len-5]) | htons(VLAN_CFI);
		i1=len-4;
		i2=0;
		while (i1<=5) {
			ctx->flow.dl_src[i2++]=ctx->flow.dl_dst[i1++];
		...
		ctx->flow.vlan_tci |= htons(ctx->flow.dl_src[len+1]) | htons(VLAN_CFI);
		i1=len+2;
		i2=0;
		while (i1<=5) {
			ctx->flow.dl_src[i2++]=ctx->flow.dl_src[i1++];
		...
		while (i1<=5) {
          ...
          if (ult_salto>0) {ctx->flow.dl_dst[i2]=ult_salto;}  //Insere no caminho o ultimo salto inserido em VLAN_VID
        }
       ...
       ////////////////////ipv4 tratamento de pacotes - fim
  
  
         ////////////////////ipv6 tratamento de pacotes - inicio
  
   else if (ctx->flow.dl_type == htons(ETH_TYPE_IPV6)) {
      ...
      if (pcp==1){ //CAMINHO ALTERNATIVO
        ...
        if (ofport->up.pp.config==1){//descartar
            ...
            ctx->flow.vlan_tci &= ~htons(VLAN_VID_MASK);//Limpa valor de VLAN_VID em VLAN_TCI
	    ctx->flow.vlan_tci &= ~htons(VLAN_PCP_MASK);//Limpa valor de VLAN_PCP em VLAN_TCI
            ctx->flow.vlan_tci |= htons(ctx->flow.ipv6_src.s6_addr[0]) | htons(VLAN_CFI)|32; // Insere em VLAN_TCI o numero da porta de envio do frame e sinaliza com VLAN_PCP = 1
	    
            
           ctx->flow.ipv6_src.s6_addr[0]=ctx->flow.ipv6_src.s6_addr[1];
           ctx->flow.ipv6_src.s6_addr[1]=ctx->flow.ipv6_src.s6_addr[2];
           ctx->flow.ipv6_src.s6_addr[2]=ctx->flow.ipv6_src.s6_addr[3];    
           ctx->flow.ipv6_src.s6_addr[3]=ctx->flow.ipv6_src.s6_addr[4];
           ctx->flow.ipv6_src.s6_addr[4]=ctx->flow.ipv6_src.s6_addr[5];
           ctx->flow.ipv6_src.s6_addr[5]=ctx->flow.ipv6_src.s6_addr[6];
	   ctx->flow.ipv6_src.s6_addr[6]=ctx->flow.ipv6_src.s6_addr[7];
	   ctx->flow.ipv6_src.s6_addr[7]=ctx->flow.ipv6_src.s6_addr[8];
	   ctx->flow.ipv6_src.s6_addr[8]=ctx->flow.ipv6_src.s6_addr[9];
	   ctx->flow.ipv6_src.s6_addr[9]=ctx->flow.ipv6_src.s6_addr[10];
	   ctx->flow.ipv6_src.s6_addr[10]=ctx->flow.ipv6_src.s6_addr[11];
	   ctx->flow.ipv6_src.s6_addr[11]=ctx->flow.ipv6_src.s6_addr[12];
	   ctx->flow.ipv6_src.s6_addr[12]=ctx->flow.ipv6_src.s6_addr[13];
	   ctx->flow.ipv6_src.s6_addr[13]=ctx->flow.ipv6_src.s6_addr[14];
	   ctx->flow.ipv6_src.s6_addr[14]=ctx->flow.ipv6_src.s6_addr[15];
	   ctx->flow.ipv6_src.s6_addr[15]=ctx->flow.ipv6_dst.s6_addr[0];
	   ctx->flow.ipv6_dst.s6_addr[0]=ctx->flow.ipv6_dst.s6_addr[1];
           ctx->flow.ipv6_dst.s6_addr[1]=ctx->flow.ipv6_dst.s6_addr[2];
           ctx->flow.ipv6_dst.s6_addr[2]=ctx->flow.ipv6_dst.s6_addr[3];    
           ctx->flow.ipv6_dst.s6_addr[3]=ctx->flow.ipv6_dst.s6_addr[4];
           ctx->flow.ipv6_dst.s6_addr[4]=ctx->flow.ipv6_dst.s6_addr[5];
           ctx->flow.ipv6_dst.s6_addr[5]=ctx->flow.ipv6_dst.s6_addr[6];
	   ctx->flow.ipv6_dst.s6_addr[6]=ctx->flow.ipv6_dst.s6_addr[7];
	   ctx->flow.ipv6_dst.s6_addr[7]=ctx->flow.ipv6_dst.s6_addr[8];
	   ctx->flow.ipv6_dst.s6_addr[8]=ctx->flow.ipv6_dst.s6_addr[9];
	   ctx->flow.ipv6_dst.s6_addr[9]=ctx->flow.ipv6_dst.s6_addr[10];
	   ctx->flow.ipv6_dst.s6_addr[10]=ctx->flow.ipv6_dst.s6_addr[11];
	   ctx->flow.ipv6_dst.s6_addr[11]=ctx->flow.ipv6_dst.s6_addr[12];
	   ctx->flow.ipv6_dst.s6_addr[12]=ctx->flow.ipv6_dst.s6_addr[13];
	   ctx->flow.ipv6_dst.s6_addr[13]=ctx->flow.ipv6_dst.s6_addr[14];
	   ctx->flow.ipv6_dst.s6_addr[14]=ctx->flow.ipv6_dst.s6_addr[15];
	   ctx->flow.ipv6_dst.s6_addr[15]=0;	 
        }
	  ...
	  len=ctx->flow.ipv6_src.s6_addr[0];
	  
	  ofp_port=ctx->flow.ipv6_src.s6_addr[1];
	  ofport = get_ofp_port(ctx->ofproto, ofp_port);
	  odp_port = ofp_port_to_odp_port(ofp_port);
          ctx->flow.vlan_tci &= ~htons(VLAN_VID_MASK);//Limpa valor de VLAN_VID em VLAN_TCI
	  ctx->flow.vlan_tci |= htons(ctx->flow.ipv6_src.s6_addr[2]) | htons(VLAN_CFI);// Insere em VLAN_TCI o numero da porta de envio do frame
            
          i1=3;
	  i2=0;
	  while (i1<=15) {
		ctx->flow.ipv6_src.s6_addr[i2++]=ctx->flow.ipv6_src.s6_addr[i1++];
	  }
	  
	  i1=0;
	  while ((i1<=15)&&(i2<=15)) {
	 	ctx->flow.ipv6_src.s6_addr[i2++]=ctx->flow.ipv6_dst.s6_addr[i1++];
	  }
	  i2=0;
	  while (i1<=15) {
	 	ctx->flow.ipv6_dst.s6_addr[i2++]=ctx->flow.ipv6_dst.s6_addr[i1++];
	  }
          
	   ctx->flow.vlan_tci &= ~htons(VLAN_PCP_MASK);//Limpa valor de VLAN_PCP em VLAN_TCI
           ctx->flow.vlan_tci |= 32;//sinaliza com VLAN_PCP = 1
	   
	  pcp = ctx->flow.vlan_tci &  htons(VLAN_PCP_MASK);
	  pcp = pcp>>5;
	  pcp &= 7; 	   
         }
	    ...
	    ctx->flow.vlan_tci |= htons(ctx->flow.ipv6_src.s6_addr[len+1]) | htons(VLAN_CFI);
	    i1=len+2;
	    i2=0;
	    while (i1<=15) {
		ctx->flow.ipv6_src.s6_addr[i2++]=ctx->flow.ipv6_src.s6_addr[i1++];
   ...
   ////////////////////ipv6 tratamento de pacotes - fim
